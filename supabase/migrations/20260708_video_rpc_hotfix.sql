...
create table if not exists public.generations (
  ...
  kind text not null default 'video_scene',
...
create index if not exists generations_active_idx on public.generations (status, updated_at desc);
...
begin
  ...
  if v.status not in ('failed', 'cancelled', 'succeeded') then
    ...
    update public.profiles
    set credits = credits + v.cost_credits,
        updated_at = now()
    where id = v.user_id;
  end if;
end;
...
begin
  ...
  where user_id = v_user_id
    ...
    and created_at >= now() - make_interval(secs => greatest(p_window_seconds, 60));
